...
import {
  Plus, ChevronRight, AlertCircle, ArrowRight,
  CheckCircle2,
} from 'lucide-react'
import { Card, Button, SectionHeader, Modal } from '@/components/ui'
import { DEMO_NEAR_MISSES } from '@/lib/demoData'
import type { NearMiss, NearMissStep, NearMissScene } from '@/types'
import {
  NEAR_MISS_SCENE_LABELS,
  NEAR_MISS_STEP_CONFIG,
} from '@/types'
...
const NearMissDetail: React.FC<{ nm: NearMiss; onClose: () => void }> = ({ nm, onClose }) => {
  ...
  const handleAdvance = () => {
    toast.success('改善ステップを進めました（デモ）')
    onClose()
  }

  const nextStep = STEP_FLOW[STEP_FLOW.indexOf(nm.step) + 1]
  const nextStepLabel = nextStep ? NEAR_MISS_STEP_CONFIG[nextStep].label : null

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${stepCfg.color}`}>
          {stepCfg.label}
        </span>
        <span className="text-xs text-gray-500">
          {format(new Date(nm.occurred_at), 'yyyy年M月d日', { locale: ja })} 発生
        </span>
        <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">
          {NEAR_MISS_SCENE_LABELS[nm.scene]}
        </span>
      </div>
      ...
      <div className="space-y-3">
        ...
        {nm.why_it_happened ? (
          ...
          <div className="bg-orange-50 rounded-lg px-3 py-2">
            <p className="text-xs text-orange-700">「なぜ起きたか」がまだ記入されていません</p>
          </div>
        )}
        ...
        {nm.recheck_date ? (
          <div>
            ...
            <p className="text-sm text-gray-800">
              {format(new Date(nm.recheck_date), 'yyyy年M月d日', { locale: ja })}
            </p>
          </div>
        ) : null}
      </div>
...
const NewNearMissForm: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleSave = () => {
    ...
    setSaving(true)
    setTimeout(() => {
      toast.success('ヒヤリハットを記録しました（デモ）')
      setSaving(false)
      onClose()
    }, 800)
  }
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="例：3歳児が砂場の縁につまずき、転倒しそうになった。近くにいた保育士がすぐに支えたため怪我はなかった。"
...
export const NearMiss: React.FC = () => {
  ...
  const sorted = [...DEMO_NEAR_MISSES].sort(
    (a, b) => stepOrder[a.step] - stepOrder[b.step]
  )
  ...
  return (
    <div className="px-4 py-6 space-y-5">
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400">
                      {format(new Date(nm.occurred_at), 'M月d日', { locale: ja })}
                    </span>
                    <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">
                      {NEAR_MISS_SCENE_LABELS[nm.scene]}
                    </span>
            ...
            {completed.map((nm) => (
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-sm text-gray-700 flex-1 break-anywhere line-clamp-2">
                    {nm.what_happened}
                  </p>
                </div>
      ...
      >
        {selected && (
          <NearMissDetail nm={selected} onClose={() => setSelected(null)} />
        )}
      </Modal>
